            ...
            <div className="card-body py-4">
                ...
                <form className="row g-3 mb-5" onSubmit={handleCreate}>
                    ...
                    <div className="col-md-2 d-flex align-items-end">
                        <button type="submit" className="btn btn-primary w-100">Them</button>
                    </div>
                </form>

                <div className="mb-4 p-3 rounded" style={{ background: "#f5f8fa" }}>
                    <div>So hinh thuc kham: <strong>{totalTypes}</strong></div>
                    <div>Tong muc gia danh muc: <strong>{Number(totalPrice).toLocaleString("vi-VN")} đ</strong></div>
                </div>
                    ...
                    <table className="table align-middle table-row-dashed fs-6 gy-3">
                        <thead>
                            <tr className="text-start text-gray-900 fw-bolder fs-7 text-uppercase">
                                ...
                                <th style={{ width: "220px" }}>Gia</th>
                                <th style={{ width: "140px" }}>Trang thai</th>
                                <th style={{ width: "220px" }} className="text-end">Thao tac</th>
                            </tr>
                        </thead>
                        <tbody>
                            {examinationTypes.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="text-center text-muted py-6">Chua co hinh thuc kham nao.</td>
                                </tr>
                            ) : (
                                    ...
                                    return (
                                        <tr key={item.id}>
                                            ...
                                            <td className="text-end">
                                                {isEditing ? (
                                                    <>
                                                        <button type="button" className="btn btn-sm btn-success me-2" onClick={saveEdit}>Luu</button>
                                                        <button type="button" className="btn btn-sm btn-light" onClick={resetEdit}>Huy</button>
                                                    </>
                                                ) : (
                                                    <>
                                                        <button type="button" className="btn btn-sm btn-primary me-2" onClick={() => startEdit(item)}>Sua</button>
                                                        <button
                                                            type="button"
